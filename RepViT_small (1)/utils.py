...
def read_split_data_mode(root: str, mode: str = 'train', supported = [".jpg", '.JPG', '.png', '.PNG', '.JPEG']):
    assert os.path.exists(root), "dataset root: {} does not exist.".format(root)
    assert mode in ['train', 'val'], "mode should be either 'train_100' or 'val_100'."
    ...
    # 将 index： class 写入指定 json 文件中
    json_str = json.dumps(dict((val, keys) for keys, val in class_indices.items()), indent=4)
    with open('class_indices.json', 'w') as json_files:
        json_files.write(json_str)

    images_path = [] # 存储所有图片路径
    images_label = [] # 存储图片对应索引信息
    every_class_num = [] # 存储每个类别样本总数

    # 遍历每个文件夹中的文件
    for cla in dataset_class:
        cla_path = os.path.join(dataset_dir,cla)
        # 遍历获取 supported 支持的所有文件路径
        images = [os.path.join(dataset_dir, cla, i) for i in os.listdir(cla_path) if os.path.splitext(i)[-1] in supported]
        images.sort()
        image_class = class_indices[cla]
        # 记录该类别样本数量
        every_class_num.append((len(images)))
